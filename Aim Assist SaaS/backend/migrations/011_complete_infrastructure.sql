...
-- 0. Create crm_type enum if it doesn't exist
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS idx_extraction_logs_tenant_lead ON extraction_logs(tenant_id, lead_id);
...
CREATE INDEX IF NOT EXISTS idx_lead_qualifications_tenant_lead ON lead_qualifications(tenant_id, lead_id);
CREATE INDEX IF NOT EXISTS idx_lead_qualifications_qualified ON lead_qualifications(is_qualified, qualified_at);
...
CREATE INDEX IF NOT EXISTS idx_usage_metrics_tenant_period ON usage_metrics(tenant_id, billing_period_start, billing_period_end);
CREATE INDEX IF NOT EXISTS idx_usage_metrics_type ON usage_metrics(tenant_id, metric_type, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_crm_integrations_sync ON crm_integrations(is_active, last_sync_at);

-- Ensure only one primary CRM per tenant using a partial unique index
CREATE UNIQUE INDEX IF NOT EXISTS idx_one_primary_per_tenant ON crm_integrations(tenant_id) WHERE is_primary = TRUE;
...
DROP POLICY IF EXISTS "Tenants can view own extraction logs" ON extraction_logs;
...
DROP POLICY IF EXISTS "Tenants can insert own extraction logs" ON extraction_logs;
...
DROP POLICY IF EXISTS "Tenants can update own extraction logs" ON extraction_logs;
...
DROP POLICY IF EXISTS "Tenants can view own lead qualifications" ON lead_qualifications;
...
DROP POLICY IF EXISTS "Tenants can manage own lead qualifications" ON lead_qualifications;
...
DROP POLICY IF EXISTS "Tenants can view own usage" ON usage_metrics;
...
DROP POLICY IF EXISTS "Tenants can view own CRM integrations" ON crm_integrations;
...
DROP POLICY IF EXISTS "Tenants can manage own CRM integrations" ON crm_integrations;
...
DROP TRIGGER IF EXISTS update_extraction_logs_updated_at ON extraction_logs;
