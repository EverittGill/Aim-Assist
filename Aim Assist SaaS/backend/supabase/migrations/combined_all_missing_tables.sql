...
-- Unique index for primary CRM will be created after table exists
...
-- Create unique index for primary CRM after table exists (only if column exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 
    ...
    IF NOT EXISTS (
      SELECT 1 
      FROM pg_indexes 
      WHERE indexname = 'idx_one_primary_crm_per_org'
    ) THEN
